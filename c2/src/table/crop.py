...
# ---------------------------------------------------------------------------
_SEAM_K = 2.5   # 缝高门槛(×行缝中位)。下限由表内分组空隙定(K=2 碎成过多段),上限由密排
#                 小表真缝定(K=3 粘连);2.4~2.5 平台无差,与"真缝=行缝2.5~10倍"吻合。
#                 标题-表空隙(3.0×+)切开无害,标题段由分类收编。
_SEAM_VMAX = 2
...
def _row_bounds(dark, dark180=None, k=_SEAM_K, vmax=_SEAM_VMAX):
    ...
    H, W = dark.shape
    # **严格白**(与 geom 行估计同尺:0.05%×宽+floor3):行号/标签行有墨→不算白被排除在带外
    # (阶梯稀疏区空隙只剩 1.4~2.3×),缝中标签自动劈开带、大空独立成缝、标签随下表。
    # 算行墨前**先去贯穿竖线列**(对称 geom.row_bnds):有框表框线给每行贡献~10px墨,不去则
    # 无行能过严格白、bands 全空(白带0条被早退跳过)。旧松白(0.3%W)需整套"贯穿否决",已删。
    keep = None
...
#   不对称原则:表格归文字=致命(数据丢),文字归表格=无害(OCR 读出 td<10 → Stage II 降级,
#   代价 1 次调用)→ 阈值取"毫无疑义是文字"的下界 30(junk 簇 ≤54、真表 ≥78;30~60 交 API)。
#   无高度门槛(高个说明块照抓)。2D熵已弃(量的是"填框"非结构,紧裁大字标题会漏)。
...
def _peel_title(im, bb):
    ...
    # 行带用 row_bnds(与行计数同尺,绝对墨判白):相对阈 _content_segs(0.4%×宽≈15px/行)看不见
    # 小标签("X岁"每行十几px墨)——标签对 peel 隐形却被行计数进去,堆叠小表家族全 +1 行。
    # row_bnds 的行带里标签作为 band0 现身 → is_data 判非数据 → 剥走。
    ...
    if k == 0 or k >= len(bands):                          # 无标题/全非数据 → 不剥
        return [], bb
    if k > 3:
        # 深标题栈(>3行)复核:旧版一刀切拒绝(深标题全留表内被读碎),现改证据门控——前缀带
        # **全部非数据行样**(_datalike:≥2成分且各居单列=三角顶稀疏签名)才剥。浅栈(≤3)零复核
        # (一年验证行为,且列对齐判据在宽列表上会误否决真标题:caption片段恰好各居宽列)。
