...
def column_cuts(dark, dark180):
    ...
    **不做 Otsu**——Otsu 双峰会被超宽 margin 缝骗、把真列缝误当窄峰滤掉致塌缩;floor10 保守
    ...
    # 最高"(平手取列多)的 floor——逐表自适应,治**系统性宽白河**(大字号表
    # 白河 12-20px 全净、比小表真缝还宽,固定阈/净列/近邻中位全失灵)。
    W = dark.shape[1]
...
def row_bnds(dark, dark180):
    ...
    while y < H:
        if white[y]:
            ...
            # 高不足=字符内假白缝;横线=分隔证据(字符内绝不含横线,
            # 且线周常有2~7px脏噪、无净行);无横线时须含净行(min<CLEAN)——
            # 细笔画中段2px假缝到不了净(否则劈开数字)
    ...
    # 斜线字形内的真 0 墨 dip 连净行检查都过(如"7"斜杠尾被隔成假行)。
