...
LINE_COVER = 0.5        # 宽松线阈:错剔无害场合(定界/检测,剔多了不丢内容)
LINE_FULL = 0.9         # 严格线阈:错剔致命场合(空白判据剔线——密字行frac0.5~0.7,
#                         0.5阈会把整行密字当线剔掉致内容蒸发;真线贯通≈1.0)
WHITE_NOISE_FRAC = 0.0005  # 白线噪声地板:允许 0.05%×线长 的噪点墨(随长度缩放)。白判据统一
#                            为 绝对墨 < max(floor, WHITE_NOISE_FRAC×线长),行 floor=3(行缝常有
#                            脏墨:噪点/水印/线周残渣,降2实测4张粘连)/列 floor=0(列缝近乎绝对
#                            干净,加容忍放脏假缝:0.01→0.0005 列精确 19→58)
ROW_WHITE_FLOOR = 3     # 行白 floor(见上)
COL_WHITE_FLOOR = 0     # 列白 floor(见上)
ROW_CLEAN_ABS = 2       # 净行:墨<此(0~1px)。真行缝内必有净行;字符内假白缝(细笔画中段
#                         恰 2px,笔画贯穿)到不了 0——白段须含净行才算真缝(否则劈开数字);
...
def column_cuts(dark, dark180):
    ...
    **不做 Otsu**——Otsu 双峰会被超宽 margin 缝骗、把真列缝误当窄峰滤掉致塌缩;floor10 保守
    ...
    # **列距均匀性选阈**(替代近邻杀弱):真列缝产生近匀距的列,白河/标签字缝混入破坏匀距。
    # 枚举缝宽 floor(≥COL_GAP_MIN),取"区间落在中位±30% 占比最高"(平手取列多)的 floor——逐表
    # 自适应,治系统性宽白河(大字号表白河 12-20px 全净、比小表真缝还宽,固定阈/净列全失灵)。
    W = dark.shape[1]
...
def col_bnds(dark, dark180):
    ...
    单侧开边(边框缺一侧)不补——与行的决策一致,交 OCR 定。

    **无条件剔除首行带**:首行常是跨列文本/表头(colspan 标题、说明),其字符间隙在主体
    无墨区变成假列缝(首行字缝把列数吹爆;+1~+4 家族多为首行文字)。列结构由
    **主体数据行**定——首行对齐时剔除无损,不对齐时剔除除害;个别真列只靠首行有墨会
    ...
    if len(rb) >= 4:                                  # ≥3行才剔首行(更小的表不折腾)
        ...
        # 尾列凭空消失(尾列被整体剔掉);主体文本右界 xR 之外保留首行作列证据。
...
def row_bnds(dark, dark180):
    ...
    稀释成"白",阶梯表下半几十行漏光);0.0005W 是噪声地板
    ...
    if len(runl) > 1 and len(runl) >= FRAME_GATE * len(gapr):
        # 不做"开边补界":个别开放边版式(行上画线、末行不封底)会少估 1 行,
    ...
    while y < H:                                     #   真行缝≥3px,真字行同样≥3px——空白带
        if not white[y]:                             #   中 1 行 3px 噪墨自成假行)
            s = y
    ...
    while y < H:
        if white[y]:
            ...
            while y < H and white[y]:
                y += 1
            # 内部白带且高≥ROW_GAP_MIN,且 [含横线行 或 含净行] → 边界取中心。高不足=字符内
            # 假白缝;横线=分隔证据(字符内绝不含横线,且线周常有脏噪无净行);无横线须含净行
            # (min<CLEAN)——细笔画中段2px假缝到不了净(否则劈开数字)
    ...
    # (斜线字形内真 0 墨 dip 连净行检查都过)。杀弱者:含横线行的赢(分隔铁证),否则更高的赢,
    # 平手杀后者,迭代至无近邻对。pitch 取表自身边界间距中位(混 1~2 假不动,自适应无外部常数)。
    changed = True
...
def split_table_texts(im, hi_frac=0.05, wd_frac=0.40):
    ...
    # 一行/列裁掉 1px(1px 底框线恰在最后一行,被裁 → 行数-1)
