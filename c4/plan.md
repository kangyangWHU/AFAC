...
- ✅ **年份路由（确定性改进）**：财报命名带年份(annual_xxx_2024_report)，route.py 增加 "ask 含年份且恰一篇 doc 含该年 → 导向那篇"。
  治财报跨年取值混淆：fin_a_003 原本两年都取成 28.75%(2025值), 修后 2024=24.29%/2025=28.75% → A ✅(确定性验证)。安全: 多篇同年则跳过, 不误导研报/同年法规。
- ✅ **多级标题/层级解析（用户: 保留层级段落关系）**：`base.py heading_level`(章节/数字 2.1/中文一、/括号（一）, 保守防数字行误判) + structure.py/pdf_pymupdf.py 层级 section_path 栈(同级替换)。
  研报 pack2_text11 抽出 `1.行情回顾/1.1指数情况/2.行业跟踪/2.1全球云基础设施…`(用户举的例子), outline 有真目录了。
- ✅ **封面/图片页 OCR enrichment（用户: 前几页重要+图片该OCR+按位置成块）**：`script/ocr_image_pages.py`。
  混合策略: PyMuPDF 抽文字页(留图表文字), MinerU 只补图片页(封面标题/扫描)。逐页检测(封面p0有图强制+低文字图片页)→抽单页PDF→MinerU**批跑一次**(避免每次重载模型50s)→按页插回。
  全量 63 篇/160 图片页 OCR 进库; 研报封面真标题找回(光模块/银保/银行IT 主题进了文档→帮内容路由)。
  **关键发现**: MinerU 非绝对更好——强在封面/版面/真表格/位置, 但**图表内部数字它存成图不OCR**(PyMuPDF 噪但抓到); 故混合(只在MinerU净赚的图片页用)才对。MinerU 1.6s/页(RTX4090), 全量5.2h离线, 无时间限制可接受。
- ✅ **全量 100 真基线（本地 Qwen3.6-35B-A3B）+ 关键 token 修复**：
  - **token 截断 bug**: 本地模型啰嗦, synth 600/judge 400 把答案行/JSON 截断→落"A"兜底。调大(synth 2000/judge 700) → **52.2%→69.6% high-conf(+16分)**。
  - **并行 facts**(solver.fact_workers): 一题内 facts 独立→并行, 喂饱端点(GPU曾仅24%)。
  - **决定性对照(同模型/索引/100题)**: 旧管线 reasoner **88.0%**(101调用) vs agentic **69.6%**(941调用)。旧的高18分且省9倍调用; agentic 唯一优势=可审计。
  - **用户决定**: 坚持 agentic——旧管线黑盒改不动/不可审/拿不了奖; agentic 每错可归因可改进。
  - **29 错题归因(agentic 可审, 旧管线做不到)**: 检索缺口16 / synthesize判错12; 漏选17/过选7/混合4。found-rate 81%(本地弱模型 vs 百炼91-98%)。多步放大弱模型误差; 换强模型差距缩小。
- ✅ **genQuery 去文档定位噪声**: ask 含公司名/年份(比亚迪/2024年)在已路由单篇内是噪声, 把排序带偏(现金流块从#1挤出top4)。genQuery 改为只留文档内区分词(指标/条款/数字)。
  现金流类财报事实从"没找到"→found; found-rate 81%→84%, 整体 69.6%→71.7%。
